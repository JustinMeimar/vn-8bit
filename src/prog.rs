        ...
        match op {
           ...
           "addi" => {
            ...
            "jmp" => {
                ...
                let op = 0x90;
                let addr = self.parse_12b_addr(v[1]);
            ...
            // Default case
            _ => println!("default, non instruction")
        } 
    }
    
    pub fn parse_12b_addr(&mut self, bits: &str) -> u16{
        let addr: &str = &bits.replace("0x", "");
        println!("{}", addr);
        let a64: i64 = i64::from_str_radix(addr, 10).unwrap();
        println!("{}", a64);
